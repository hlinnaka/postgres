 ...
 * NOTES:
 *
 * - careful with sending interrupts to wrong process
 * - if the same interrupt is sent multiple times, they are coalesced. That's
 *   pretty efficient
 *
 *
 ...
 * LWLock waits), so they should react in good time. They can be suppressed by
 ...
 *
 * Multiplexed INTERRUPT_WAIT_WAKEUP
 * ---------------------------------
 *
 *
 ...
 * -----------------------
 *
 * The correct pattern to wait for event(s) using INTERRUPT_WAIT_WAKEUP is:
 *
 * for (;;)
 * {
...
{
	/*
	 * INTERRUPT_WAIT_WAKEUP is multiplexed for many reasons, XXX
	 *
	 * Examples:
	 * -
	 *
	 * Please also use CHECK_FOR_INTERRUPTS to avoid uninterruptible sleeps
	 */
	INTERRUPT_WAIT_WAKEUP = 1 << 0,
	 ...
	 * when more WAL arrives, or when promotion is requested.
	 */
