...
if (1)
{
	...
	for(my $i = 0; $i < $num_connections; $i++) {
		...
		$primary_session->query_safe($consume_xids_query);
...
# TEST: A mix of a handful of different subxids
if (1)
{
	$primary->safe_psql('postgres', 'CREATE TABLE tbl(i int)');
	$primary->safe_psql('postgres', "INSERT INTO tbl SELECT g FROM generate_series(1, 100000) g;");
	...
	for(my $i = 0; $i < $num_subxacts; $i++) {
		$primary->safe_psql('postgres', $consume_xids_query);
	...
	repeat_and_time_sql("few-subxacts-wide-apart", $replica, "select count(*) from tbl");
...
# TEST 2: many-subxacts-wide-apart
if (1)
{
	$primary->safe_psql('postgres', 'CREATE TABLE tbl(i int)');
	$primary->safe_psql('postgres', "INSERT INTO tbl SELECT g FROM generate_series(1, 100000) g;");
