			  ...
			  BlockNumber *old_nblocks, BlockNumber *nblocks)
{
	...
	for (i = 0; i < nforks; i++)
	{
		 ...
		 *
		 * We can have nblocks > oldblocks when a relation was truncated
		 * multiple times, a replica applied all truncations and restarts with
		 * the restartpoint located before the truncates. The relation on disk
		 * will be the size of the last truncate. When replaying the first
		 * truncate, we will have nblocks > current size. With those values,
		 * smgr_truncate will do nothing, so we need to restore the cached
		 * size to the old_nblocks size.
		 */
