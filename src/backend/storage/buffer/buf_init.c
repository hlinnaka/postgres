...
BufferManagerShmemRequest(void *arg)
{
	...
	BufferDescriptorsShmemDesc.size = NBuffers * sizeof(BufferDescPadded);
	ShmemRequestStruct(&BufferDescriptorsShmemDesc);

	BufferBlocksShmemDesc.size = NBuffers * (Size) BLCKSZ;
	ShmemRequestStruct(&BufferBlocksShmemDesc);

	BufferIOCVArrayShmemDesc.size = NBuffers * sizeof(ConditionVariableMinimallyPadded);
	 ...
	 * painful.
	 */
	CkptBufferIdsShmemDesc.size = NBuffers * sizeof(CkptSortItem);
