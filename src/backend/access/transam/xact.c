 ...
 * first, and its each subtransaction comes after its parent.  Because of how
 * XIDs are assigned to subtransaction, so you can binary search the array if
 * you use wraparound-aware comparisons (i.e. TransactionIdPrecedes).
 *
 ...
 * When a subtransaction is committed, its totalXids is copied to the parent,
 * so that the parent absorbs all the child XIDs.  When a subtransaction is
 * rolled back, the subtransaction's totalXids is just forgotten and you
 * revert to using the parent's totalXids.
 */
