...
BackendStatusShmemRequest(void *arg)
{
	...
	BackendStatusArrayShmemDesc.size =
		mul_size(sizeof(PgBackendStatus), NumBackendStatSlots);
	ShmemRequestStruct(&BackendStatusArrayShmemDesc);

	BackendAppnameBufferShmemDesc.size =
		mul_size(NAMEDATALEN, NumBackendStatSlots);
	ShmemRequestStruct(&BackendAppnameBufferShmemDesc);

	BackendClientHostnameBufferShmemDesc.size =
		mul_size(NAMEDATALEN, NumBackendStatSlots);
	ShmemRequestStruct(&BackendClientHostnameBufferShmemDesc);

	BackendActivityBufferSize = mul_size(pgstat_track_activity_query_size,
										 NumBackendStatSlots);
	BackendActivityBufferSizeShmemDesc.size = BackendActivityBufferSize;
...
#ifdef USE_SSL
	{
		...
		BackendSslStatusBufferShmemDesc.size =
...
#ifdef ENABLE_GSS
	{
		...
		BackendGssStatusBufferShmemDesc.size =
